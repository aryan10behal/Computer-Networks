...
class Node {
 ...
 public:
  ...
  {
      for(auto &table: mytbl.tbl) 
      {

        // searching the routing table of router to update cost to destination ip = 16 (infinite distance)
        if(table.dstip == destination_ip)
        {
          table.cost = 16;
        }
      }
      ...
      {
        // removing the interface so that it does not participate in future iterations.
        if(interface.first.getConnectedIp() == destination_ip)
        {
  ...
  bool find_unreachable()
  {
    for (int i = 0; i < mytbl.tbl.size(); ++i) {
      if(mytbl.tbl[i].cost>=16)
        return true;
    }
    return false;
  }
  void sendMsg(){
    ...
    for (auto interface: interfaces) {
        ntbl.tbl.clear();
      for(auto entries: mytbl.tbl)
      {
        ntbl.tbl.push_back(entries);
      }
      for(int j = 0; j<ntbl.tbl.size();j++)
      {
        ...
        string my_table_next_hop = mytbl.tbl[j].nexthop;
        if(interface.second->isMyInterface(my_table_next_hop) == true)
          ntbl.tbl[j].cost = 16;
        msg.mytbl = &ntbl;	
